...
max_iter_inc=16 # Last iter to increase #Gauss on.
...
retry_beam=40
totgauss=16000 # Target #Gaussians.
careful=false
...
stage=-4
power=0 # exponent to determine number of gaussians from occurrence counts
...
sdata=$data/split$nj;
[[ -d $sdata && $data/feats.scp -ot $sdata ]] || split_data_new.sh $data $nj || exit 1;
